...
1. **Intentional Vulnerabilities**: This software contains intentionally implanted security vulnerabilities including broken access control, insecure direct object references, privilege escalation flaws, encoding/crypto bypasses, and weak signature verification.
...
1. **故意漏洞**：本软件包含故意植入的安全漏洞，包括但不限于越权访问、不安全的直接对象引用、权限提升缺陷、编码/加密绕过和弱签名验证。
...
| ID | Category | Description |
|----|----------|-------------|
| E-01~E-05 | Encoding Bypass | Parameters encoded with Base64/Base32/Caesar/Custom/Multi-layer, bypassing frontend validation |
| E-06 | Encryption Bypass | AES-256-GCM encrypted parameters with hardcoded key exposure |
| E-07 | Signature Bypass | HMAC-SHA256 signature verification with leaked key |
| E-08 | National Crypto Bypass | SM4-CBC encrypted parameters with key exposure |
| E-09 | Hash Signature Bypass | MD5 hash signature with predictable salt |
