...
func decodeEncodedID(c *gin.Context, encType string) (uint, error) {
	...
	switch encType {
	case "base64":
		decoded, err = cryptopkg.Base64Decode(encodedID)
	case "base32":
		decoded, err = cryptopkg.Base32Decode(encodedID)
	case "base58":
		decoded, err = cryptopkg.Base58Decode(encodedID)
	case "custom_base64":
		decoded, err = cryptopkg.CustomBase64Decode(encodedID)
	case "caesar":
		decoded = cryptopkg.CaesarDecode([]byte(encodedID), 3)
	case "aes":
		decoded, err = cryptopkg.AESDecrypt(encodedID, []byte("demo-key-32-bytes-long-1234567"))
	case "sm4":
		decoded, err = cryptopkg.SM4Decrypt(encodedID, []byte("demo-sm4-key-16b"))
	default:
		decoded, err = cryptopkg.Base64Decode(encodedID)
	}
...
func (h *EncodedHandler) EncodeValue(c *gin.Context) {
	...
	case "caesar":
		result = string(cryptopkg.CaesarEncode([]byte(input.Value), 3))
	case "aes":
		// Use a fixed demo key for the utility endpoint
		result, err = cryptopkg.AESEncrypt([]byte(input.Value), []byte("demo-key-32-bytes-long-1234567"))
	case "sm4":
		result, err = cryptopkg.SM4Encrypt([]byte(input.Value), []byte("demo-sm4-key-16b"))
	case "hash":
		result = cryptopkg.ComputeHashSign(input.Value, "demo-salt")
	default:
...
func (h *EncodedHandler) DecodeValue(c *gin.Context) {
	...
	switch input.EncodingType {
	case "base64":
		decoded, err = cryptopkg.Base64Decode(input.Value)
	case "base32":
		decoded, err = cryptopkg.Base32Decode(input.Value)
	case "base58":
		decoded, err = cryptopkg.Base58Decode(input.Value)
	case "custom_base64":
		decoded, err = cryptopkg.CustomBase64Decode(input.Value)
	case "caesar":
		decoded = cryptopkg.CaesarDecode([]byte(input.Value), 3)
	case "aes":
		decoded, err = cryptopkg.AESDecrypt(input.Value, []byte("demo-key-32-bytes-long-1234567"))
	case "sm4":
		decoded, err = cryptopkg.SM4Decrypt(input.Value, []byte("demo-sm4-key-16b"))
	default:
		common.BadRequest(c, "Unsupported encoding type: "+input.EncodingType)
		return
	}
...
func (h *EncodedHandler) GetCryptoKeys(c *gin.Context) {
	common.Success(c, gin.H{
		"aes_key_base64":  cryptopkg.Base64Encode([]byte("demo-key-32-bytes-long-1234567")),
		"sm4_key_base64":  cryptopkg.Base64Encode([]byte("demo-sm4-key-16b")),
		"hash_salt":       "demo-salt",
	})
}
